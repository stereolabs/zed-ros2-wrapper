...
v5.5.0
------
- Added the `general.bus_type` parameter to the `ZedCameraOne` node (`AUTO`, `USB`, `GMSL`, and with ZED SDK >= 5.5 `MIPI` and `HOLOSCAN`), used when the camera is selected with `general.camera_id`. The bus was hard-coded to `GMSL`, and the ZED SDK matches the bus type against the camera transport exactly, so a ZED X One Core connected to a MIPI capture card or reached over Camera-over-Ethernet through a Holoscan sensor bridge could not be opened by ID at all. The default is now `AUTO`, which searches every bus and matches what the `general.serial_number` path already did.
- Added the `pause_svo_rec` service (stereo and mono, `std_srvs/srv/SetBool`) to pause and resume an SVO recording without closing the file, using `sl::Camera::pauseRecording()`. Only `start_svo_rec` and `stop_svo_rec` were available, so skipping an uninteresting part of a run meant closing the file and starting a new one. Calling it while no recording is active returns `success: false`. The diagnostic now reports `SVO Recording: PAUSED` instead of `ACTIVE` while the recording is paused.
- Added the `general.max_system_clock_step_ms` parameter (stereo and mono), forwarded to `sl::setMaxSystemClockStepMs()`. It sets how fast the SDK follows a backward host-clock adjustment (NTP/PTP step, manual date change) in `SYSTEM_CLOCK` mode: `4.0` is the ZED SDK default and suits slew-only setups, a negative value makes step corrections land instantly as PTP deployments expect, and `0.0` freezes the offset captured on first use. It is applied only when `general.sdk_timestamp_clock` is `SYSTEM_CLOCK`, since a monotonic clock never steps. Requires ZED SDK >= 5.3.
- Added the `general.sdk_timestamp_clock` parameter (stereo and mono) to select the clock backing every SDK timestamp: `SYSTEM_CLOCK` (default), `MONOTONIC_CLOCK` or, with ZED SDK >= 5.4, `MONOTONIC_RAW_CLOCK`. The latter is immune to NTP/PTP frequency slewing as well as to clock steps, which matters when correlating SDK timestamps with a free-running hardware counter or a device the host does not discipline. It replaces the boolean `general.sdk_use_monotonic_clock`, which could only reach `MONOTONIC_CLOCK`: the boolean is still read, and still works, but now logs a deprecation warning.
- Added the `svo.encryption_key` parameter (stereo and mono) to encrypt the SVO files created by the `start_svo_rec` service, forwarded to `RecordingParameters::encryption_key` (AES-256-CTR). Playback of encrypted SVO files was already supported through `svo.decryption_key`, but there was no way to produce one from the wrapper. The value is a path to a 32-byte key file, a 64-character hex key, or a passphrase, and it is never written to the log: the node only reports whether encryption is enabled. Requires ZED SDK >= 5.3 and OpenSSL at runtime.
- Added the positional tracking pose confidence to the node output. `sl::Pose::pose_confidence` is a [0,100] quality metric of the pose estimation, filled by `POSITIONAL_TRACKING_MODE::GEN_3`, where 0 means the tracking is lost and 100 that it can be fully trusted. It is reported as `Pose confidence` in the diagnostic, and published as the new `pose_confidence` field of the `PosTrackStatus` message on `~/pose/status` when the matching `zed_msgs` v5.5 is installed. Requires ZED SDK >= 5.4.
- Fixed the `distortion_model` and the distortion coefficients published in the `camera_info` topics. They were chosen from the camera model with a hard-coded table, which the lens does not follow: a fisheye ZED X One was published as `rational_polynomial` instead of `equidistant`, and the ZED X HDR models were not in the table at all, so their `camera_info` carried an empty distortion model and no coefficients. Both nodes now read `sl::CameraParameters::lens_distortion_model` (ZED SDK >= 5.4) and fall back to the coefficient-based heuristic on older versions. Rectified parameters are reported as `plumb_bob` with zero coefficients, instead of repeating the raw model with zeroed values.
- Aligned the `zed_debug` configuration files with the `zed_wrapper` ones. The debug configuration was missing `depth.voxel_point_cloud`, `depth.voxel_size_mm`, `depth.voxel_resolution_mode`, `depth.voxel_resolution_scale` and `general.grab_compute_capping_fps`, and its `custom_object_detection.yaml` lagged four per-class tracking parameters behind. The two directories now declare exactly the same parameters; only the debug-specific values still differ.
- Fixed `general.sdk_use_monotonic_clock` sitting in the `debug` section of `zed_wrapper/config/common_mono.yaml` and `zed_debug/config/common_stereo.yaml`. Both nodes read it as `general.sdk_use_monotonic_clock`, so in those two files the parameter was ignored and setting it to `true` had no effect.
- Added the missing `advanced.change_thread_priority` to `common_stereo.yaml` and to both `common_mono.yaml` files. It gates every other `advanced.*` parameter and defaults to `false`, so the thread scheduling policy and priorities listed next to it were never applied unless it was added by hand.
- Fixed the `sports ball` class in `custom_object_detection.yaml`, whose label was corrupted to `pesports ballrson` by a bad search and replace. All the other 79 labels and every `model_class_id` were verified against the COCO 2017 order.
- Improved the per-class default values of `custom_object_detection.yaml`. `is_grounded` is now true only for objects that really lie on the floor plane (it was set on tableware, food, laptops and other objects sitting on tables, whose 3D position was then projected onto the floor), `is_static` only for objects fixed in the world (a computer mouse was declared static), and `tracking_timeout`, the 3D box size bounds, the velocity smoothing, the velocity threshold and the confirmation and prediction times are now set per object type instead of being identical for the 80 classes.
- Fixed `object_detection.class_XXX.min_box_height_meters` being ignored: the parameter was documented in `custom_object_detection.yaml` for every class but never read by the node, so the minimum 3D height was never applied.
- Added `CUSTOM_RFDETRLIKE_BOX_OBJECTS` and `CUSTOM_BOX_OBJECTS_AUTODETECT` as valid `object_detection.detection_model` values (ZED SDK >= 5.5). They run a user-provided ONNX file inside the ZED SDK like `CUSTOM_YOLOLIKE_BOX_OBJECTS`, and now take the same wrapper path: the `custom_object_detection.yaml` parameters, the custom per-class properties and the raw labels. Previously they were accepted by the parameter parser but treated as built-in models, so the ONNX file was never passed to the ZED SDK and enabling object detection failed.
- Fixed the ONNX input shape sent to the ZED SDK for custom object detection models. `object_detection.custom_onnx_input_size` was assigned to `ObjectDetectionParameters::custom_onnx_dynamic_input_shape` as a bare integer, which built a `sl::Resolution(size, 0)` instead of a square `size x size` because of the default arguments of `sl::Resolution`. The height reaching the ZED SDK was always 0, so the shape never matched the one a model had been optimized with and the engine had to be rebuilt.
- Added the camera health status to the diagnostic output. The `Health: ...` fields report image quality, lighting, depth reliability, motion sensors reliability and, with ZED SDK >= 5.5 and `zed_msgs` v5.5, duplicated images; the diagnostic goes to WARN and names the issues while any of them is raised. The fields report `DISABLED` when `general.enable_image_validity_check` is off, since the ZED SDK does not compute the checks then. The status is now read once per grab and cached, so the health topic and the diagnostic updater report the same values without querying the SDK from two threads.
- Added the `duplicated_image` field to the `HealthStatusStamped` message published on `~/status/health`. It is raised by the ZED SDK when the content of a frame is identical to one already received even though its timestamp is new, i.e. a repeating or stalled stream, which until now was only mentioned in the verbose log. Like the other health checks it needs `general.enable_image_validity_check` enabled (default). Requires ZED SDK >= 5.5 and the matching `zed_msgs` v5.5: with an older `zed_msgs` the node builds and runs as before, without the field.
- Added the `general.timestamp_reference` parameter (`IMAGE`, `IMAGE_CENTER_OF_EXPOSURE`; default `IMAGE`) to both the stereo and the mono node. `IMAGE_CENTER_OF_EXPOSURE` stamps the published data with the middle of the frame exposure instead of the start of the sensor readout, i.e. half an exposure earlier, which is what aligns frames with sensors timestamped at the instant they measure (LiDAR, robot joints). It needs an input carrying a per-frame exposure (ZED X, ZED X Mini, ZED X One GS, ZED X One 4K, live/SVO/stream): on USB and HDR models the ZED SDK returns 0, so the node warns once and falls back to `IMAGE` for the rest of the session. Sensors data, the SVO `/clock` topic and simulation keep using `IMAGE`. Requires ZED SDK >= 5.5.
- Changed the default `body_tracking.body_format` from `BODY_38` to `BODY_34`, and the default `body_tracking.enable_body_fitting` from `false` to `true`. Only `BODY_18` and `BODY_34` have a `GEN_2` network, so the previous default silently fell back to the older `GEN_1` network; `BODY_34` is the closest format to `BODY_38` that runs `GEN_2`, and it requires body fitting (the ZED SDK forces it on otherwise, now with a warning from the node). Skeletons published on `~/body_trk/skeletons` carry 34 keypoints instead of 38: set `body_format` back to `BODY_38` to keep the previous topology, at the cost of running `GEN_1`.
- Added the `body_tracking.model_gen` parameter (`GEN_1`, `GEN_2`; default `GEN_2`) to select which body tracking network runs. `GEN_2` is the new bottom-up architecture: more robust in crowded scenes and to unusual body orientations, it detects people further from the camera, and it is up to 18% faster and 14% more accurate than `GEN_1`. Only `BODY_18` and `BODY_34` have a `GEN_2` network, so the node warns and the ZED SDK falls back to `GEN_1` with the default `body_tracking.body_format: 'BODY_38'`. `body_tracking.allow_reduced_precision_inference` has no effect on `GEN_2`, which always runs in FP16. Requires ZED SDK >= 5.5.
- Added the `pos_tracking.compute_preference` parameter (`AUTO`, `PREFER_CPU`, `PREFER_GPU`; default `AUTO`) to control how much GPU the positional tracking may use. `GEN_3` runs on the CPU by default, but some of its operations have a GPU implementation too: `PREFER_GPU` makes tracking faster and lowers the per-frame time of `grab()`, at the cost of using the GPU, while `PREFER_CPU` keeps the GPU free for your own workloads. `GEN_1` computes depth and so uses the GPU whatever the setting. Requires ZED SDK >= 5.5.
- Added the `depth.allow_depth_cuda_graph` parameter (default `false`). When enabled, the ZED SDK records the depth computation once and replays it at every grab, lowering the CPU cost of launching it — most useful on Jetson, where that overhead is a larger share of the frame time. The depth output is unchanged. Best-effort: it only applies to the `NEURAL` depth mode family, and the SDK reverts to the regular computation for the rest of the session if the recording fails. Requires ZED SDK >= 5.5.
- Added the `depth.depth_precision` parameter to select the precision of the neural depth inference: `FP16` (default, unchanged behavior) or `INT8`, which is faster and uses less GPU memory for a small accuracy cost. `INT8` is only available with `depth.depth_mode: 'NEURAL'`; the ZED SDK falls back to `FP16` when the depth mode or the GPU does not support it. Requires ZED SDK >= 5.5.
